...
import type { DatasetInfo, JsonObject } from "./types.js";
...
export const FORMAT_GUIDANCE = {
  ...
  preferred_jsonl_shape: {
    file: "input.jsonl（也接受 .json 数组或 records/items 包装）",
    row: "taskName, queryId, query, versionAName, versionBName, responseA, responseB, productCardsA, productCardsB, traceA?, traceB?",
...
export function jsonlCheckPayload(inputPath: string, continueCommand: string): JsonObject {
  ...
  if (![".json", ".jsonl", ".ndjson"].includes(suffix)) {
    issues.push(issue("INPUT_SUFFIX_REQUIRED", "error", "输入文件必须使用 .json、.jsonl 或 .ndjson 后缀", { path }, "平台按这些后缀识别统一 A/B 输入。", "使用支持的后缀后重试。", continueCommand));
    return { ok: false, type: "aidp_jsonl_check", path, row_count: 0, issues };
  }
  ...
  const records: Array<{ row: unknown; location: string }> = [];
  if (suffix === ".json") {
    try {
      const parsed = JSON.parse(content) as unknown;
      let values: unknown[];
      if (Array.isArray(parsed)) values = parsed;
      else if (isPlainObject(parsed) && Array.isArray(parsed.records)) values = parsed.records;
      else if (isPlainObject(parsed) && Array.isArray(parsed.items)) values = parsed.items;
      else values = [parsed];
      values.forEach((row, index) => records.push({ row, location: `记录 ${index + 1}` }));
    } catch (error) {
      issues.push(issue("INPUT_PARSE_ERROR", "error", "JSON 文件无法解析", { error: error instanceof Error ? error.message : String(error) }, "JSON 必须是 object、array 或 records/items 包装。", "修正 JSON 语法。", continueCommand));
    }
  } else {
    content.split(/\r?\n/).forEach((line, index) => {
      if (!line.trim()) return;
      try {
        records.push({ row: JSON.parse(line), location: `第 ${index + 1} 行` });
      } catch (error) {
        issues.push(issue("INPUT_PARSE_ERROR", "error", `第 ${index + 1} 行不是有效 JSON`, { line: index + 1, error: error instanceof Error ? error.message : String(error) }, "每个非空行必须是一个 JSON object。", "修正该行 JSON 语法。", continueCommand));
      }
    });
  }
  for (const record of records) {
    const { row, location } = record;
    if (!isPlainObject(row)) {
      issues.push(issue("INPUT_ROW_NOT_OBJECT", "error", `${location}顶层不是 object`, { location }, "平台按字段读取每一道题。", "把该记录改为 JSON object。", continueCommand));
      continue;
    }
    const missing = JSONL_REQUIRED_STRINGS.filter((key) => typeof row[key] !== "string" || !String(row[key]).trim());
    if (missing.length) {
      issues.push(issue("INPUT_REQUIRED_FIELD_INVALID", "error", `${location}缺少必填字符串字段`, { location, fields: missing }, "统一输入需要完整的题目、版本和回复字段。", "补齐 evidence.fields。", continueCommand));
      continue;
    }
    const queryId = String(row.queryId);
    if (!SAFE_QUERY_ID.test(queryId)) {
      issues.push(issue("INPUT_QUERY_ID_UNSAFE", "error", `${location} queryId 不合法`, { location, query_id: queryId }, "queryId 必须稳定且可安全索引。", "使用字母、数字、点、下划线或连字符，长度不超过 200。", continueCommand));
      continue;
    }
    if (seen.has(queryId)) {
      issues.push(issue("INPUT_DUPLICATE_QUERY_ID", "error", `queryId 重复：${queryId}`, { location, query_id: queryId }, "重复题目会覆盖原始证据。", "保证 queryId 在文件内唯一。", continueCommand));
      continue;
    }
    for (const key of ["productCardsA", "productCardsB"]) {
      ...
      if (!Array.isArray(cards) || cards.some((card) => typeof card !== "string" || !isJsonObjectString(card))) {
        issues.push(issue("INPUT_PRODUCT_CARDS_INVALID", "error", `${location} ${key} 必须是 JSON 字符串数组`, { location, field: key }, "这与 AIDP 的列表字段 contract 一致。", "无商品卡时传 []；有卡时每项使用 JSON.stringify 后的 object。", continueCommand));
      }
    }
    ...
    if (rowCount && (current[0] !== taskName || current[1] !== versionAName || current[2] !== versionBName)) {
      issues.push(issue("INPUT_HEADER_INCONSISTENT", "error", `${location}的任务名或版本名不一致`, { location, expected: { taskName, versionAName, versionBName } }, "一个输入文件对应一个固定的 A/B 任务。", "统一所有记录的 taskName/versionAName/versionBName。", continueCommand));
    }
  ...
  return { ok, type: "aidp_jsonl_check", message: ok ? "JSON/JSONL 数据检查通过" : "JSON/JSONL 数据检查失败", path, row_count: rowCount, task_name: taskName, version_names: { A: versionAName, B: versionBName }, issues };
}
...
export function inspectDatasetDir(inputPath: string): DatasetInfo {
  const path = datasetPath(inputPath);
  ...
  if (pathA && pathB) {
    ...
    if (validA.size && validB.size && !common.length) {
      issues.push(issue(
        "ZERO_COMMON_ITEMS",
        "error",
        "A/B 版本没有同名 JSON 文件，评估任务会是 0 条",
        pair,
        "平台把去掉 .json 后的文件名作为 query_id，只展示两个版本目录内都存在的同名 JSON。",
        "把同一条 case 在两个版本目录内保存为完全相同的文件名，例如 item_0001.json。",
        continueCommand,
      ));
    }
    if (onlyA.length || onlyB.length) {
      issues.push(issue(
        "UNMATCHED_JSON_IGNORED",
        "warning",
        "部分 JSON 只存在于一个版本中，会被平台忽略",
        pair,
        "这不会阻塞上传，但这些 case 不会进入 A/B 对比评估。",
        "补齐缺失版本，或删除不参与评估的单边文件。",
        continueCommand,
      ));
    }
  }
...
function issuesForDataset(info: DatasetInfo, label: string, continueCommand: string): JsonObject[] {
  ...
  if (!info.exists) {
    items.push(issue(
      ...
      "error",
      `找不到${label}`,
      { path: info.path, dataset_role: label },
      "平台只能上传或绑定真实存在的目录。",
      "确认路径是否拼写正确，或先完成数据转换输出。",
      continueCommand,
    ));
    return items;
  }
  if (!info.is_dir) {
    items.push(issue(
      ...
      `${label}不是目录`,
      { path: info.path, dataset_role: label },
      "平台上传和绑定的单位是数据集文件夹，不是单个文件。",
      "把每条 case 拆成独立 JSON 文件，放入一个版本目录。",
      continueCommand,
    ));
    return items;
  }
  ...
  if (!info.json_files.length) {
    ...
    items.push(issue(
      info.convertible_files.length ? "NON_JSON_INPUT" : "NO_JSON_FILES",
      "error",
      `${label}的第一层没有 .json 文件`,
      {
        path: info.path,
        dataset_role: label,
      ...
      "服务端只读取版本目录第一层的 .json 文件；CSV/XLSX/JSONL 不会被平台自动转换。",
      conversionPlan
        ? "按 evidence.conversion_plan 为每个源文件创建一个版本目录；把每条记录写成一个 JSON object 文件，并用同一个稳定字段生成文件名。"
        : "如果 JSON 在子目录里，请把应参与评估的 .json 文件移动到该版本目录第一层；如果当前目录只有源数据文件，请先转换。",
      continueCommand,
    ));
  }
  if (info.invalid_json_files.length) {
    items.push(issue(
      "JSON_PARSE_ERROR",
      "error",
      `${label}中有 JSON 文件无法解析`,
      { dataset_role: label, count: info.invalid_json_files.length, samples: sample(info.invalid_json_files, 10) },
      "解析失败的文件不会被服务端加载，可能导致评估条数变少或变成 0。",
      "修正这些文件的 JSON 语法后重新检查。",
      continueCommand,
    ));
  }
  if (info.non_object_json_files.length) {
    items.push(issue(
      ...
      "当前服务端按 object 读取字段；数组、字符串或数字顶层会被跳过。",
      "把每条 case 转成一个 JSON object，例如 {\"query\":\"...\",\"response\":\"...\"}。",
      continueCommand,
    ));
  }
  if (info.nested_json_files.length) {
    items.push(issue(
      "NESTED_JSON_IGNORED",
      "warning",
      `${label}存在子目录 JSON，平台会忽略`,
      { dataset_role: label, count: info.nested_json_files.length, samples: sample(info.nested_json_files) },
      "服务端使用目录第一层的 *.json 作为数据集内容，不递归扫描。",
      "如这些文件应该参与评估，请移动到版本目录第一层。",
      continueCommand,
    ));
  }
  if (info.non_json_files.length) {
    items.push(issue(
      "NON_JSON_FILES_IGNORED",
      "warning",
      `${label}存在非 JSON 文件，平台会忽略`,
      { dataset_role: label, count: info.non_json_files.length, samples: sample(info.non_json_files) },
      "上传接口只保存 .json 文件；其他文件不会进入评估。",
      "如这些是源数据，请先转换为 JSON 文件夹结构。",
      continueCommand,
    ));
  }
  if (info.valid_json_files.length && info.recognized_default_field_files === 0) {
    items.push(issue(
      "CUSTOM_RENDERER_LIKELY_REQUIRED",
      "warning",
      `${label}的 JSON 没有默认 renderer 常用字段`,
      { dataset_role: label, sample_keys: Object.fromEntries(sample(Object.entries(info.sample_keys), 3)) },
      "默认 renderer 主要展示 query、response、product_cards 和 trace；其他字段需要自定义 renderer 才能清晰展示。",
      "上传任务 renderer.js，或在转换 JSON 时补充 query/response 字段。",
...
function buildConversionPlan(info: DatasetInfo): JsonObject | null {
  ...
  if (analyses.length >= 2) {
    ...
    for (const key of ["uuid", "case_uuid", "case_id", "id", "source_id", "meta.source_id"]) {
      const sets = analyses.map((analysis) => analysis._candidate_sets[key] as Set<string> | undefined);
      if (sets.every(Boolean)) {
        const [first, ...rest] = sets as Array<Set<string>>;
...
function analyzeJsonlFile(path: string, limit = 2000): JsonObject & { _candidate_sets: Record<string, Set<string>> } {
  const candidates = ["uuid", "case_uuid", "case_id", "id", "source_id", "meta.source_id"];
  const values = Object.fromEntries(candidates.map((key) => [key, new Set<string>()])) as Record<string, Set<string>>;
...
function findNestedJson(root: string): string[] {
  const out: string[] = [];
...
function suggestVersionDirName(fileName: string, index: number): JsonObject {
...
function safeOutputName(value: string, fallback = "version"): string {
