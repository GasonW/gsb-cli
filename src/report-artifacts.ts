...
export function bundleReportArtifacts(html: string, reportPath: string, workspaceArg?: string): {
  html: string; files: Record<string, string>;
} {
  const files: Record<string, string> = {};
  let root = workspaceArg ? resolve(workspaceArg) : dirname(resolve(reportPath));
  if (!workspaceArg) {
    while (basename(root) !== 'workspace' && dirname(root) !== root) root = dirname(root);
  }
  ...
  const rendered = html.replace(/<a\b[^>]*>/gi, tag => {
    ...
    if (!evidence && !classes.includes('artifact-download')) return tag;
    if (attrs.has('data-artifact-parts')) return tag;
    const web = attrs.get('data-web-href') || '';
    if (/^(?:\.\/)?artifact-[a-f0-9]{64}-/.test(web)) return tag;
    ...
    if (!workspacePath) throw new Error('Report attachment is missing its workspace path');
    if (!workspaceArg && basename(root) !== 'workspace') throw new Error('Report attachments require --workspace-root');
    if (!/^(tasks|model_runs|benchmarks|annotation_sets)\//.test(workspacePath)) throw new Error(`Unsupported artifact path: ${workspacePath}`);
    ...
    if (!/\.(jsonl|json)$/i.test(path)) throw new Error('Report artifacts must be JSON or JSONL');
    if (!existsSync(path)) throw new Error(`Missing artifact: ${workspacePath}`);
    const bytes = readFileSync(path);
    const content = bytes.toString('utf8');
    if (!Buffer.from(content).equals(bytes)) throw new Error('Artifact must be valid UTF-8');
    const digest = createHash('sha256').update(bytes).digest('hex');
    if (evidence && attrs.get('data-sha256') !== digest) throw new Error('Report evidence checksum mismatch');
    const name = evidence ? `evidence-${digest}.json` : `artifact-${digest}-${basename(path)}`;
    if (evidence && bytes.length > 32 * 1024 * 1024) throw new Error('A report evidence file exceeds 32 MiB');
