...
从命令 JSON 输出读取 `review_report`、`report`、`cqc_report` 和 `summary`，再归档到平台：
...
- 未修改分数表示认可当前统计来源分；未操作评论在保存时默认采纳。
- “保存 Review”表示该题完成；页面实时更新 `Review / 筛选 / All` 数量和按 Review 状态筛选的结果。
...
该命令只读取并导出已保存的题目终判、评论状态以及按题目和 Reviewer 汇总的统计；它不启动 Review 页面、不写入 Review 结果，也不触发算法/CQC 报告重算。
