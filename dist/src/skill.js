import { cpSync, existsSync, lstatSync, mkdirSync, realpathSync, readFileSync, rmSync, symlinkSync, } from "node:fs";
...
export function skillTargets(target, env = process.env) {
    const labels = target === "all" ? ["codex", "cursor"] : [target];
    return labels.map((label) => {
        const root = targetRoot(label, env);
        return { target: label, root, dest: join(root, SKILL_NAME) };
    });
}
export function readSkillVersion(path) {
...
export function skillInfo(target, env = process.env) {
...
export function installSkill(target, mode, force, env = process.env) {
...
export function uninstallSkill(target, env = process.env) {
    const results = [];
    for (const item of skillTargets(target, env)) {
...
function targetRoot(target, env) {
