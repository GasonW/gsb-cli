...
import { datasetCheckPayload, datasetPath, FORMAT_GUIDANCE, validFileMap } from "./dataset.js";
    ...
    try {
        ...
        if (error instanceof ApiError) {
            ...
            if ([400, 401].includes(error.status) && ["用户不存在", "密码错误", "Username and password required"].includes(server)) {
                const item = issue("AUTH_CREDENTIALS_REQUIRED_OR_INVALID", "error", "登录凭据缺失或不可用", { status: error.status, server_error: server, has_username: Boolean(username), has_password: Boolean(password) }, "登录 API 需要有效的 username 和 password。", "确认用户名和密码是否正确；如账号被重置或密码过期，请先处理账号状态后重新登录。", redactedArgv(globals.rawArgv));
                return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
            }
...
async function cmdTaskCreate(globals, args) {
    ...
    const client = await buildClient(globals);
    try {
        const data = await client.request("POST", "/api/tasks", { name, mode, task_id: taskId });
...
async function cmdTaskGet(globals, args) {
    ...
    const client = await buildClient(globals);
    try {
        const data = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}`);
...
async function uploadOneDataset(client, info, name) {
    return client.request("POST", "/api/datasets/upload", {
