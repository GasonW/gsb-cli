    ...
    try {
        ...
        return {
            payload: {
                ok: false,
                message: "CLI 执行失败",
                issues: [issue("CLI_INTERNAL_ERROR", "error", error instanceof Error ? error.message : String(error), {}, "CLI 遇到未处理异常。", "保留命令输出并联系 CLI 维护者。")],
            },
            exitCode: 1,
        };
    }
}
async function dispatch(globals, args) {
    ...
    if (command === "skill") {
        if (subcommand === "install")
            return cmdSkillInstall(globals, rest);
        if (subcommand === "status")
            return cmdSkillStatus(globals, rest);
        if (subcommand === "uninstall")
            return cmdSkillUninstall(globals, rest);
    }
    if (command === "auth") {
        if (subcommand === "login")
            return cmdAuthLogin(globals);
        if (subcommand === "register")
            return cmdAuthRegister(globals);
        if (subcommand === "whoami")
            return cmdAuthWhoami(globals);
        if (subcommand === "logout")
            return cmdAuthLogout(globals);
    }
    if (command === "dataset") {
        if (subcommand === "verify-task")
            return cmdVerifyTask(globals, rest);
        if (subcommand === "check")
            return cmdDatasetCheck(globals, rest);
        if (subcommand === "upload")
            return cmdDatasetUpload(globals, rest);
        if (subcommand === "list")
            return cmdDatasetList(globals);
        if (subcommand === "guide")
            return { payload: { ok: true, message: "JSON 数据结构说明", format_guidance: FORMAT_GUIDANCE }, exitCode: 0 };
    }
    if (command === "task") {
        if (subcommand === "create")
            return cmdTaskCreate(globals, rest);
        if (subcommand === "get")
            return cmdTaskGet(globals, rest);
        if (subcommand === "create-gsb")
            return cmdTaskCreateGsb(globals, rest);
        if (subcommand === "bind")
            return cmdTaskBind(globals, rest);
        if (subcommand === "setup")
            return cmdTaskSetup(globals, rest);
        if (subcommand === "configure")
            return cmdTaskConfigure(globals, rest);
        if (subcommand === "config")
            return cmdTaskConfig(globals, rest);
    ...
    if (command === "results") {
        if (subcommand === "summary")
            return cmdResultsSummary(globals, rest);
        if (subcommand === "export")
            return cmdResultsExport(globals, rest);
    }
        ...
        if (error instanceof ApiError) {
            return {
                payload: {
                    ...
                    base_url: client.baseUrl,
                    reachable: false,
                    issues: [issue(error.status === 0 ? "PLATFORM_UNREACHABLE" : "PLATFORM_API_INCOMPATIBLE", "error", "doctor 检查失败", { status: error.status, url: error.url, server_error: serverError(error.data) }, "CLI 通过 --base-url 访问服务器端平台；该检查失败说明网络、认证入口或平台 API 有问题。", "确认 base URL、网络连通性和平台版本。", redactedArgv(globals.rawArgv))],
                },
                exitCode: 1,
            };
        }
...
async function cmdVersion(globals, args) {
...
function cmdSkillStatus(globals, args) {
    const reader = new OptionReader(args);
...
function cmdSkillInstall(globals, args) {
    const reader = new OptionReader(args);
    const target = readSkillTarget(reader.takeString("target", "all"));
    ...
    try {
        ...
        const item = issue("SKILL_INSTALL_FAILED", "error", error instanceof Error ? error.message : String(error), { target, mode }, "skill 安装需要写入本机 Agent skills 目录。", "确认目标目录可写；如果需要覆盖已有 skill，请加 --force。", redactedArgv(globals.rawArgv));
        return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
    }
}
function cmdSkillUninstall(globals, args) {
    const reader = new OptionReader(args);
    const target = readSkillTarget(reader.takeString("target", "all"));
    reader.requireNoUnknown();
    return {
...
function readSkillTarget(value) {
...
async function cmdAuthLogin(globals) {
    const username = globals.username;
    const password = globals.password || globals.env.GSB_PASSWORD;
    if (!username || !password) {
        const item = issue("AUTH_CREDENTIALS_REQUIRED_OR_INVALID", "error", "登录凭据缺失或不可用", { has_username: Boolean(username), has_password: Boolean(password) }, "登录 API 需要 username 和 password。", "提供可用的用户名和密码；密码应通过环境变量或安全参数注入，不要写入仓库文件。", "gsb-cli auth login --username <user>");
        return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
    }
    ...
    try {
        ...
        if (data.force_change_pw) {
            payload.issues = [issue("PASSWORD_CHANGE_REQUIRED", "warning", "当前账号被要求修改默认密码", { username: String(data.username || username) }, "网页端会要求修改密码；CLI 目前不处理改密流程。", "在网页完成密码修改，或让管理员重置为可用密码后重新登录。", "gsb-cli auth login --username <user>")];
        }
        ...
        if (error instanceof ApiError) {
            ...
            if ([400, 401].includes(error.status) && ["用户不存在", "密码错误", "Username and password required"].includes(server)) {
                const item = issue("AUTH_CREDENTIALS_REQUIRED_OR_INVALID", "error", "登录凭据缺失或不可用", { status: error.status, server_error: server, has_username: Boolean(username), has_password: Boolean(password) }, "登录 API 需要有效的 username 和 password。", "确认用户名和密码是否正确；如账号被重置或密码过期，请先处理账号状态后重新登录。如确认还没有账号，可先运行 gsb-cli auth register --username <user> --password <pass> --json。", redactedArgv(globals.rawArgv));
                return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
            }
...
async function cmdAuthRegister(globals) {
    const username = globals.username;
    const password = globals.password || globals.env.GSB_PASSWORD;
    if (!username || !password) {
        const item = issue("AUTH_REGISTER_CREDENTIALS_REQUIRED", "error", "注册凭据缺失", { has_username: Boolean(username), has_password: Boolean(password) }, "注册 API 需要 username 和 password。CLI 不会自动生成账号，避免误注册。", "先向用户确认要注册的账号和密码；密码至少 6 位，建议通过 GSB_PASSWORD 注入。", "gsb-cli auth register --username <user>");
        return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
    }
    const baseUrl = resolveBaseUrl(globals);
    const client = new ApiClient({ baseUrl });
    try {
        ...
        if (error instanceof ApiError) {
            const server = serverError(error.data);
            if (error.status === 400) {
                const item = issue("AUTH_REGISTER_FAILED", "error", "注册失败", { status: error.status, server_error: server, has_username: Boolean(username), has_password: Boolean(password) }, "平台注册接口拒绝了该账号信息，常见原因是用户名已存在、用户名格式不合法或密码少于 6 位。", "按 server_error 修正账号信息；如果已有账号，改用 gsb-cli auth login。", redactedArgv(globals.rawArgv));
                return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
            }
...
async function cmdAuthWhoami(globals) {
...
async function cmdAuthLogout(globals) {
    const client = await buildClient(globals, { autoLogin: false });
    try {
...
async function cmdVerifyTask(globals, args) {
    const reader = new OptionReader(args);
    ...
    if (!input || !output || !task || !acceptance)
        throw new CliUsageError("verify-task requires TASK --input --output --acceptance");
    if (existsSync(output))
        throw new CliUsageError("readback output already exists");
    const text = readFileSync(input, "utf8");
    const audit = JSON.parse(readFileSync(acceptance, "utf8"));
    const hash = createHash("sha256").update(text).digest("hex");
    if (audit.local_pass !== true || audit.input_sha256 !== hash)
        throw new CliUsageError("acceptance failed or input changed");
    const check = jsonlCheckPayload(input, redactedArgv(globals.rawArgv));
    if (!check.ok)
        return { payload: check, exitCode: 1 };
    const rows = text.split(/\r?\n/).filter((line) => line.trim()).map((line) => JSON.parse(line));
    const client = await buildClient(globals, { autoLogin: false });
    const status = await client.request("GET", `/api/tasks/${encodeURIComponent(task)}/status`);
    const datasets = status.datasets;
    const counts = datasets.counts;
    if (counts.common !== rows.length || counts.a !== rows.length || counts.b !== rows.length)
        throw new CliUsageError("server row count differs");
    const remote = [];
    for (const row of rows) {
        for (const side of ["A", "B"]) {
            const data = await client.request("GET", `/tasks/${encodeURIComponent(task)}/api/source?query=${encodeURIComponent(String(row.queryId))}&version=${side}`);
            const actual = JSON.parse(String(data.content));
            if (!isDeepStrictEqual(actual, row))
                throw new CliUsageError(`server input mismatch: ${String(row.queryId)} ${side}`);
            if (side === "A")
                remote.push(actual);
        }
    }
...
function cmdDatasetCheck(globals, args) {
    const reader = new OptionReader(args);
    const input = reader.takeOptionalString("input");
...
async function cmdDatasetUpload(globals, args) {
    const reader = new OptionReader(args);
    const input = reader.takeOptionalString("input");
    const acceptance = reader.takeOptionalString("acceptance");
    if (acceptance) {
        if (!input)
            throw new CliUsageError("--acceptance requires --input");
        const audit = JSON.parse(readFileSync(acceptance, "utf8"));
    ...
    const duplicateStrategy = strategyFlags[0];
    const client = await buildClient(globals);
    ...
    if (jsonlPath && existsSync(datasetPath(jsonlPath)) && statSync(datasetPath(jsonlPath)).isFile()) {
        ...
        try {
            ...
            if (duplicateStrategy)
                payload.on_duplicate = duplicateStrategy;
            const data = await client.request("POST", "/api/datasets/upload", payload);
            return {
                payload: {
                    ok: true,
    ...
    if (a || b) {
        if (!a || !b) {
            const item = issue("PAIR_UPLOAD_REQUIRES_A_AND_B", "error", "缺少一个版本目录", { a: a || "", b: b || "" }, "GSB 对比需要两个版本目录。", "补齐 --a 和 --b 后重新运行。", redactedArgv(globals.rawArgv));
    ...
    return {
        payload: {
            ...
            message: "数据集上传完成",
            uploaded,
            warnings: check.issues.filter((item) => item.severity === "warning"),
            next_commands: [
                "gsb-cli task create --name <task-name> --purpose <task-purpose>",
...
async function cmdDatasetList(globals) {
    const client = await buildClient(globals);
    try {
        const data = await client.request("GET", "/api/datasets");
...
async function cmdTaskCreate(globals, args) {
    const reader = new OptionReader(args);
    ...
    const client = await buildClient(globals);
    try {
        const data = await client.request("POST", "/api/tasks", { name, purpose, mode, task_id: taskId });
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "创建任务", globals);
        throw error;
    }
}
async function cmdTaskGet(globals, args) {
    ...
    try {
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "读取任务详情", globals);
        throw error;
    }
}
async function cmdTaskBind(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    ...
    try {
        if (input) {
            ...
            const data = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/select-dirs`, {
                dataset_id: ref.id,
            });
            return {
                payload: {
        ...
        if (data.data_mode === "gsb" && Number(data.common_count || 0) <= 0) {
            issues.push(issue("ZERO_COMMON_ITEMS_AFTER_BIND", "error", "数据源已绑定但共同题数为 0", { count_a: data.count_a, count_b: data.count_b, common_count: data.common_count }, "平台只展示 A/B 同名 JSON 文件。共同题数为 0 时发布会被 preflight 阻止。", "修正 A/B 文件名后重新上传并绑定。", `gsb-cli task preflight ${String(data.task_id || taskId)} --json`));
        }
        ...
        if (error instanceof DatasetRefError) {
            const item = issue(String(error.evidence.code || "DATASET_REF_ERROR"), "error", "数据集引用无法解析", error.evidence, "绑定接口需要服务端可访问的数据集路径。远端平台推荐先 dataset upload，再使用返回的 dataset id 绑定。", "先运行 dataset list，使用明确的 id；或先 dataset upload。", redactedArgv(globals.rawArgv));
            return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
        }
        if (error instanceof ApiError)
            return apiFailurePayload(error, "绑定数据源", globals);
        throw error;
    }
}
async function cmdTaskCreateGsb(globals, args) {
    const reader = new OptionReader(args);
    const name = reader.takeString("name", "");
    const purpose = reader.takeString("purpose", "");
    const taskId = reader.takeString("task-id", "");
    ...
    const client = await buildClient(globals);
    try {
        const createData = await client.request("POST", "/api/tasks", { name, purpose, mode: "gsb", task_id: taskId });
        ...
        const refs = input
            ...
            : [await resolveDatasetRef(client, a), await resolveDatasetRef(client, b)];
        ...
        const bindData = await client.request("POST", `/tasks/${encodeURIComponent(createdTaskId)}/api/select-dirs`, input
        ...
        const setupData = await client.request("POST", `/tasks/${encodeURIComponent(createdTaskId)}/api/setup`, {
            task_name: name,
            task_description: description,
            min_per_person: minPerPerson,
            anchor_count: anchorCount,
        });
        const configData = await client.request("POST", `/tasks/${encodeURIComponent(createdTaskId)}/api/admin-config`, {
            visibility: {
        ...
        const preflight = await client.request("GET", `/api/tasks/${encodeURIComponent(createdTaskId)}/preflight`);
        let publishData;
        if (publish) {
            if (!preflight.ok) {
                return {
            ...
            publishData = await client.request("POST", `/api/tasks/${encodeURIComponent(createdTaskId)}/publish`, {});
        }
        ...
        if (error instanceof DatasetRefError) {
            const item = issue(String(error.evidence.code || "DATASET_REF_ERROR"), "error", "数据集引用无法解析", error.evidence, "create-gsb 需要绑定平台可访问的数据集。远端平台推荐先 dataset upload，再使用返回的 dataset id。", "先运行 dataset upload 或 dataset list，使用明确的 dataset id。", redactedArgv(globals.rawArgv));
            return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
        }
        if (error instanceof ApiError)
            return apiFailurePayload(error, "创建 GSB 任务", globals);
        throw error;
    }
}
async function cmdTaskConfigure(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    const reader = new OptionReader(args.slice(1));
    const name = reader.takeString("name", "");
    const descriptionFile = reader.takeOptionalString("description-file");
    ...
    if (transparentMode !== undefined)
        visibility.transparent_mode = transparentMode;
    if (stats !== undefined)
    ...
    try {
        ...
        if (setupRequested) {
            ...
            const setupData = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/setup`, {
                task_name: name || String(before.task?.name || ""),
        ...
        if (configRequested) {
            const configData = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/admin-config`, { visibility });
            steps.push({ name: "config", ok: true, visibility: configData.visibility ?? visibility });
        }
        const preflight = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}/preflight`);
        ...
        if (publish) {
            ...
            publishData = await client.request("POST", `/api/tasks/${encodeURIComponent(taskId)}/publish`, {});
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "配置任务", globals);
        throw error;
    }
}
async function cmdTaskSetup(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    const reader = new OptionReader(args.slice(1));
    const name = reader.takeString("name", "");
    const descriptionFile = reader.takeOptionalString("description-file");
    ...
    const client = await buildClient(globals);
    try {
        const data = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/setup`, payload);
        ...
        const warnings = [
            issue("SETUP_ASSIGNMENT_GENERATED", "warning", "task setup 已生成并保存分配策略", {
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "配置任务分配策略", globals);
        throw error;
    }
}
async function cmdTaskConfig(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    const reader = new OptionReader(args.slice(1));
    const visibility = {};
    const transparentMode = reader.takeOptionalString("transparent-mode");
    ...
    const client = await buildClient(globals);
    try {
        const data = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/admin-config`, { visibility });
        return { payload: { ok: true, message: "权限配置已保存", visibility: data.visibility ?? visibility }, exitCode: 0 };
    }
    catch (error) {
        if (error instanceof ApiError)
            return apiFailurePayload(error, "配置任务权限", globals);
        throw error;
    }
}
async function cmdTaskPreflight(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    const client = await buildClient(globals);
    try {
        const data = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}/preflight`);
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "运行发布 preflight", globals);
        throw error;
    }
}
async function cmdTaskPublish(globals, args) {
    const taskId = requireArg(args, 0, "task-id");
    const client = await buildClient(globals);
    try {
        const preflight = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}/preflight`);
        ...
        const data = await client.request("POST", `/api/tasks/${encodeURIComponent(taskId)}/publish`, {});
        return {
            payload: {
                ok: true,
                message: "任务已发布",
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "发布任务", globals);
        throw error;
    }
}
...
async function cmdTaskRenderer(globals, args) {
    ...
    if (subcommand === "status") {
        try {
            const data = await client.request("GET", `/tasks/${encodeURIComponent(taskId)}/api/renderer`);
            return { payload: { ok: true, message: "renderer 状态", renderer: data }, exitCode: 0 };
        }
        catch (error) {
            if (error instanceof ApiError)
                return apiFailurePayload(error, "读取 renderer 状态", globals);
            throw error;
        }
    }
    if (subcommand === "upload") {
        ...
        if (!existsSync(path) || !statSync(path).isFile()) {
            const item = issue("RENDERER_FILE_NOT_FOUND", "error", "renderer 文件不存在", { path }, "上传接口需要读取本地 renderer.js 内容。", "确认文件路径，或让 Agent 先生成 renderer.js。", redactedArgv(globals.rawArgv));
            return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
        }
        const content = readFileSync(path, "utf8");
        const warnings = content.includes("renderPanel")
            ? []
            : [issue("RENDER_PANEL_NOT_FOUND", "warning", "renderer 中没有发现 renderPanel 标识", { path }, "评估页通过 renderPanel(data, ...) 渲染左右面板；缺少该函数可能导致仍使用默认渲染。", "确认 renderer.js 定义了全局 renderPanel 函数。", redactedArgv(globals.rawArgv))];
        try {
            const data = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/renderer`, { content });
            return {
                payload: {
                    ok: true,
            ...
            if (error instanceof ApiError)
                return apiFailurePayload(error, "上传 renderer", globals);
            throw error;
        }
    }
    if (subcommand === "clear") {
        try {
            const data = await client.request("DELETE", `/tasks/${encodeURIComponent(taskId)}/api/renderer`, {});
            return { payload: { ok: true, message: "已恢复全局默认 renderer", server_response: data }, exitCode: 0 };
        }
        catch (error) {
            if (error instanceof ApiError)
                return apiFailurePayload(error, "清除 renderer", globals);
            throw error;
        }
    }
...
async function cmdReportUpload(globals, args) {
    ...
    try {
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "上传归档分析报告", globals);
        throw error;
    }
}
...
async function cmdResultsSummary(globals, args) {
    ...
    try {
        const data = await client.request("GET", `/tasks/${encodeURIComponent(taskId)}/api/summary${query}`);
...
async function cmdResultsExport(globals, args) {
    ...
    const client = await buildClient(globals);
    try {
        const start = await client.request("POST", `/tasks/${encodeURIComponent(taskId)}/api/export`, { format, scope });
        const pollUrl = typeof start.poll_url === "string" ? start.poll_url : "";
        if (!pollUrl) {
            const item = issue("EXPORT_POLL_URL_MISSING", "error", "服务端导出响应缺少 poll_url", { response: start }, "CLI 无法确认导出任务何时完成。", "检查服务端导出接口返回值。", redactedArgv(globals.rawArgv));
            return { payload: { ok: false, message: "导出任务未返回 poll_url", issues: [item] }, exitCode: 1 };
        }
        const deadline = Date.now() + timeout * 1000;
        let status = {};
        while (Date.now() < deadline) {
            status = await client.request("GET", pollUrl);
            if (status.status === "done")
                break;
            if (status.status === "error") {
                const item = issue("EXPORT_JOB_FAILED", "error", "服务端导出任务失败", { status }, "导出后台任务执行出错。", "根据 error 修复后重新导出。", redactedArgv(globals.rawArgv));
                return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
            }
            await delay(interval * 1000);
        }
        if (status.status !== "done") {
            const item = issue("EXPORT_TIMEOUT", "error", "等待导出完成超时", { timeout_seconds: timeout, last_status: status }, "导出任务可能仍在后台运行，也可能卡住。", "稍后重试，或在网页任务管理页查看导出状态。", redactedArgv(globals.rawArgv));
            return { payload: { ok: false, message: item.problem, issues: [item] }, exitCode: 1 };
        }
        ...
        if (error instanceof ApiError)
            return apiFailurePayload(error, "导出评估结论", globals);
        throw error;
    }
}
async function uploadOneDataset(client, info, name, onDuplicate) {
    const payload = {
        folder_name: name || basename(info.path),
        files: validFileMap(info),
    };
    if (onDuplicate)
        payload.on_duplicate = onDuplicate;
    return client.request("POST", "/api/datasets/upload", payload);
}
function publicDatasetPayload(label, data) {
    const payload = publicDatasetRef(data);
    payload.label = label;
    payload.ok = data.ok;
    return payload;
}
function publicDatasetRef(data) {
    const out = {};
...
async function resolveDatasetRef(client, ref) {
    ...
    const grouped = await client.request("GET", "/api/datasets");
    const my = Array.isArray(grouped.my) ? grouped.my : [];
    const others = Array.isArray(grouped.others) ? grouped.others : [];
    const allItems = [...my, ...others].filter((item) => typeof item === "object" && item !== null);
...
async function buildClient(globals, options = {}) {
...
function datasetUploadFailurePayload(error, globals) {
    ...
    const item = issue(code, "error", String(detail.message || detail.error || "数据集上传冲突"), detail, "平台会阻止同名但内容不同的数据集被静默覆盖，避免后续任务绑定到错误数据。", String(detail.next_step || "确认本次上传目的后，使用 --reuse、--replace、--new-name 或 --force-new 重新上传。"), redactedArgv(globals.rawArgv));
    return { payload: { ok: false, message: item.problem, issues: [item], duplicate: detail }, exitCode: 1 };
}
function parseOptionalNumberOrAuto(name, raw) {
    if (raw === undefined) {
        return undefined;
    }
...
function defaultMinPerPerson(totalItems) {
    if (totalItems <= 0)
        return 0;
    return Math.min(totalItems, Math.max(10, Math.ceil(totalItems * 0.15)));
}
function defaultAnchorCount(totalItems, minPerPerson) {
    if (totalItems <= 0 || minPerPerson <= 0)
        return 0;
    return Math.min(totalItems, minPerPerson, Math.max(3, Math.ceil(minPerPerson * 0.10)));
}
function resolveMinPerPerson(value, totalItems) {
...
function resolveAnchorCount(value, totalItems, minPerPerson) {
    if (value === undefined || value === "auto") {
...
async function fetchTaskStatus(client, taskId) {
    try {
        return await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}/status`);
    }
    catch (error) {
        if (!(error instanceof ApiError) || error.status !== 404) {
            throw error;
        }
        const task = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}`);
        let preflight = {};
        try {
            preflight = await client.request("GET", `/api/tasks/${encodeURIComponent(taskId)}/preflight`);
        }
        catch {
            preflight = {};
        }
...
function legacyTaskStatus(taskId, task, preflight) {
...
function publicTaskFromStatus(status) {
    return isJsonObject(status.task) ? status.task : {};
}
function withTaskUrls(status, client, taskId) {
    return {
        ...status,
        urls: {
...
function summarizeSetupEffects(data) {
    const config = isJsonObject(data.config) ? data.config : {};
    const anchorItems = Array.isArray(config.anchor_items) ? config.anchor_items : [];
    const evalDimensions = Array.isArray(config.eval_dimensions) ? config.eval_dimensions : [];
    return {
...
function apiFailurePayload(error, action, globals, continueCommand = "") {
    const detail = error.data && typeof error.data === "object" ? error.data : { raw: error.data };
    const item = issue(error.status === 0 ? "API_UNREACHABLE" : "API_REQUEST_FAILED", "error", `${action}失败`, { status: error.status, url: error.url, server_error: serverError(error.data) }, "CLI 调用的是服务器端平台真实 API；该错误表示服务端拒绝了当前操作或服务不可达。", "先根据 server_error 修正认证、权限、任务状态或输入数据；如果是 401，请重新登录。", continueCommand || redactedArgv(globals.rawArgv));
    const payload = { ok: false, message: item.problem, issues: [item] };
    if (detail.preflight && typeof detail.preflight === "object") {
        payload.preflight = detail.preflight;
        const failures = detail.preflight.failures;
