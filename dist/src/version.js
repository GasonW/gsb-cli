...
export async function updateNoticeText(env = process.env) {
...
async function fetchNpmVersion(packageName) {
    ...
    const data = JSON.parse(await fetchText(`https://registry.npmjs.org/${encodeURIComponent(packageName)}/latest`));
    return { version: String(data.version || ""), source: `npm:${packageName}` };
}
async function fetchPackageJsonVersion(url) {
...
function updateCommand(source, env) {
