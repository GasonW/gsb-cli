export function issue(code, severity, problem, evidence = {}, why = "", nextStep = "", continueCommand = "") {
    const payload = {
        code,
        severity,
...
export function sample(values, limit = 8) {
    return Array.from(values).slice(0, limit);
}
export function redactedArgv(rawArgv) {
    const parts = ["gsb-cli"];
...
function continueAfterFixHint(code, command, severity) {
    if (!command) {
        return undefined;
    }
